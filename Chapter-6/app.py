...
def make_bar(slider_input, title):
    fig = go.Figure(
        data=[
            go.Bar(
                x=["Cash", "Bonds", "Stocks"], # categories on the x-axis
                y=slider_input,  # categories on the y-axis
                text=slider_input,
                textposition="inside",
                texttemplate="%{text:.2f}%", # adds percentage to number
                marker=dict
                    (color=[COLORS["cash"], COLORS["bonds"], COLORS["stocks"]]
                     ),
                hoverinfo="none",
            )
        ]
...
bonds_allocation = html.Div(
    id='bonds-allocation',
    style={'fontSize': '20px', "fontWeight": "bold", "paddingTop": "20px"},
)
...
history_card = dbc.Card(
    [
        dbc.CardHeader("User Parameter History Record"),
...
tabs = dbc.Tabs(
    [
        ...
        dbc.Tab(history_card,label="History", tab_id="history"),
    ],
    id="tabs",
    active_tab="tab-2",
    className="mt-2",
    ...
    [
        dbc.Row(
            dbc.Col(
                ...
                html.H3("Elise Reynolds - CS 150 Community Action Computing", className = "text-center"),
                    ]
            )
        ),
        dbc.Row(
            [
                ...
                dbc.Col(
                    [
                        dcc.Graph(id="bar_graph", className="mb-2"),
...
def update_bar(stocks, cash):
    # calculates the bond allocaiton as the reminaing portion
    bonds = 100 - cash - stocks
    slider_input = [cash, bonds, stocks]
    title = f"Asset Mix: Cash {cash}%, Bonds {bonds}%, Stocks {stocks}%"

    bar_figure = make_bar(slider_input, title)

    bonds_text = f"Bonds Allocation: {bonds}%"

    return bar_figure, bonds_text
